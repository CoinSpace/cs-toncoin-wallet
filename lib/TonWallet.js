...
export class TonTransaction extends Transaction {
  get url() {
    if (this.development) {
...
export class JettonTransaction extends Transaction {
  get url() {
...
export default class TonWallet extends CsWallet {
  ...
  static tokenUrl(platform, address, development) {
    if (development) {
      return `https://testnet.tonscan.org/jetton/${address}`;
    }
  ...
  async #createTransfer({ secretKey, value, address, memo, csFeeValue, csFeeAddress }) {
    ...
    const transfer = this.#wallet.createTransfer({
      secretKey,
      seqno,
      timeout: Math.floor(Date.now() / 1e3) + 3600,
  ...
  #transformCoinTx(tx) {
    if (tx.in_msg.source) {
      ...
      const action = parseJettonMsgBody(msg.msg_data?.body)?.op === 'Transfer'
        ? TonTransaction.ACTION_TOKEN_TRANSFER
        : TonTransaction.ACTION_TRANSFER;
